...
// Mock dependencies
const mockHttpClient: jest.Mocked<AxiosInstance> = {
  get: jest.fn(),
  post: jest.fn(),
  put: jest.fn(),
  delete: jest.fn(),
...
describe('VersionOperations', () => {
  let versionOps: VersionOperations;
  ...
  describe('getVersionHistory', () => {
    it('should get version history successfully', async () => {
      ...
      expect(mockHttpClient.get).toHaveBeenCalledWith(
        '/content/test/page.versionhistory.json',
  ...
  describe('restoreVersion', () => {
    it('should restore version successfully', async () => {
      ...
      expect(mockHttpClient.post).toHaveBeenCalledWith(
        '/bin/wcm/versioning/restoreVersion',
  ...
  describe('compareVersions', () => {
    it('should compare versions successfully', async () => {
      ...
      expect(mockHttpClient.get).toHaveBeenCalledWith(
        '/content/test/page.version.1.0.json',
        expect.objectContaining({
          params: { ':depth': '2' },
        })
      );
      expect(mockHttpClient.get).toHaveBeenCalledWith(
        '/content/test/page.version.1.1.json',
        expect.objectContaining({
          params: { ':depth': '2' },
        })
      );
  ...
  describe('deleteVersion', () => {
    it('should delete version successfully', async () => {
      ...
      expect(mockHttpClient.post).toHaveBeenCalledWith(
        '/bin/wcm/versioning/deleteVersion',
        expect.any(URLSearchParams),
        expect.objectContaining({
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
